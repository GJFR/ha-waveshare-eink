...
#### Method B: Using the Terminal (Advanced)
If you prefer not to remove your SD card, you can use the **Advanced SSH & Web Terminal** add-on.
1. Install the **Advanced SSH & Web Terminal** add-on from the Add-on Store.
2. In the add-on configuration, **disable "Protection Mode"** (required for host access).
3. Start the add-on and open the terminal.
4. Run the following commands to mount the boot partition and enable SPI:
   ```bash
   mkdir /tmp/boot
   # Use /dev/mmcblk0p1 for SD card or /dev/sda1 for SSD
   mount /dev/mmcblk0p1 /tmp/boot
   echo "dtparam=spi=on" >> /tmp/boot/config.txt
   ```
5. **Reboot the entire host** (Settings > System > Hardware > Reboot).
